...
function addName() { //a function that adds  names to the array,namesArray
    ...
    if (name === '') {
        alert("Please enter a name") // the if statement says if nothing is entered in the input box, an alert is given to enter a name
        return
    }
...
function displayNames() { //function that displays the names added in the namesArray
    ...
    nameList.style.fontSize = '4rem'// changed the size of the font of names that appear in javascipt
...
function pickRandomName() { // creates the function pickRandomName which will pick a random name when the button is clicked
    ...
    document.getElementById('randomName').classList = 'text-danger' // changed the color of the text to red (for names that appear when random button clicked)
    randomNameDiv.style.fontSize = '5rem' // changed the size of the font of the random names that appear in javascipt
